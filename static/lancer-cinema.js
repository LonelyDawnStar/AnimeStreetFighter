...
const LancerCinema=(()=>{
 let manifest, pages=[];
 const ready=(async()=>{
  const response=await fetch('/assets/lancer-pixel/manifest.json');
  if(!response.ok)throw Error('게이 볼그 프레임 정보 로딩 실패');
  manifest=await response.json();
  pages=await Promise.all(manifest.pages.map(name=>new Promise((resolve,reject)=>{
   const image=new Image();image.onload=()=>resolve(image);
   image.onerror=()=>reject(Error('게이 볼그 프레임 로딩 실패: '+name));
   image.src='/assets/lancer-pixel/'+name;
  })));
 })();
 ready.catch(()=>{});
 ...
 return {ready,draw,get duration(){return manifest?manifest.duration:4}};
})();
