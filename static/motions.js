...
GrailArt.promise.then(()=>document.getElementById('status').textContent=GrailArt.ready?'READY · SEVEN SERVANTS':GrailArt.error.message);
function draw(now){
 ...
 if(GrailArt.ready){
  ...
  ['saber','archer','lancer','gil','iskandar','medusa','berserker'].forEach((char,i)=>{
   const duration=action==='skill'&&char==='berserker'?1.15:action==='skill'?.5:char==='lancer'?.58:.52;
   const p={char,x:90+i*183,y:0,face,stun:0,inv:0,action,anim:0,animMax:duration};
   if(action==='heavy'||action==='skill'){p.anim=Math.max(0,duration-cycle);if(!p.anim)p.action='idle'}
   if(action==='skill'&&char==='berserker'&&p.anim>0)p.action='axe_slam';
   ...
   GrailArt.fighter(g,p,time,.59,510);
   g.fillStyle='#efce91';g.font='bold 20px monospace';g.textAlign='center';g.fillText(['SABER','EMIYA','CU CHULAINN','GILGAMESH','ISKANDAR','MEDUSA','HERACLES'][i],90+i*183,564);
