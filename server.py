...
NP_MAX_DURATION=9.0
NP_SOURCE_DURATION={'allmight':5.6,'acheron':5.2,'gojo':5.4,'saber':7.2,'archer':460/60,'lancer':6.4,'gil':7.6,'berserker':6.6,'iskandar':7.5,'medusa':7.0}
...
def bot_input(r,p,enemy,dt,rng=None):
 ...
 for shot in r['shots']:
  ...
  ult=shot['kind'] in ('np','ea_beam','greatslash','army_charge','pegasus_charge','purple','united_smash')
 ...
 if cfg['heal'] and p['seals']>0 and p['hp']<p.get('maxHp',100)*.32:
  ...
  skill_range=260 if p['char']=='allmight' else 290 if p['char']=='acheron' else 245 if p['char']=='berserker' else 320 if p['char']=='iskandar' else 510 if p['char']=='medusa' else 800
...
def character_catalog():
 return {'augments':aug.catalog(),'characters':{k:{**v,**COMBAT[k], 'hp':100,'mana':100,'manaRegen':11,'heavyDamage':v['damage']*1.7,'heavyCost':12,'heavyReach':v['reach']+30,'skillWindup':{'allmight':.42,'saber':.24,'archer':.9,'lancer':.3,'gil':0,'iskandar':.32,'medusa':.25,'berserker':.48,'gojo':.32,'acheron':.28}[k],'skillVelocity':{'allmight':0,'saber':820,'archer':1150,'lancer':1000,'gil':670,'iskandar':560,'medusa':950,'berserker':0,'gojo':850,'acheron':0}[k],'skillHits':3 if k=='gil' else 1,'npHits':0 if k in ('berserker','acheron') else 12 if k=='gil' else 1,'npEffect':'유나이티드 스테이츠 오브 스매시: 연출 6.6초 → 준비 0.5초 → 전방 340, 피해 38. 가드·회피·점프 가능. 원작 기술을 대전용으로 조정' if k=='allmight' else '다음 J/K 3회 강화: J 14 / K 20, 사거리 +85. 3회 모두 적중 시 황천의 귀환 18 추가. 헛침/가드/회피도 횟수 소모, 가드·회피는 적중 수 제외. 강화 중 재발동 불가, 라운드 종료 시 초기화. 원작 연출을 대전용으로 재구성' if k=='acheron' else '체력 12% 회복 · 6초간 피해 35% 감소 · 효과 중 치명타를 받으면 체력 20%로 재기(라운드당 1회)' if k=='berserker' else '허식 자: 창과 혁을 결합. 연출 6.4초 → 0.6초 발사 준비. 기본 고정 피해 36 · 가드/보호막 적용 · 점프/회피 가능. HP 조건 없음. 전투 수치는 대전용 조정' if k=='gojo' else '공격형 보구','npDuration':NP_TITLE_DURATION+NP_DURATION[k],'dashCost':18,'dashDistance':145,'dashInv':.17,'dashCooldown':.25,'guardReduction':82,'guardCost':6,'sealCount':3,'sealHeal':28} for k,v in CHARS.items()}}
...
def tick(r,dt,now):
 ...
 for i,p in enumerate(ps):
  ...
  beam=next((s for s in r['shots'] if s['owner']==i and (s['kind'] in ('ea_beam','greatslash','army_charge','pegasus_charge','royal_charge','axe_slam','octobolt','stygian','detroit','united_smash') or (s['kind'] in ('caladbolg','strike_air','spear_throw','chain_throw') and s['delay']>0))),None)
